            ...
            $.get(url + '/get-customer-instalment?customer_id=' + customer_id,function (data) {
                ...
                if(data == ''){
                    ...
                    $.each(data,function (index,subcatObj) {
                        $('#repayment_table > tbody').append('<tr><td>'+ subcatObj.custom+'</td><td>'+ subcatObj.total_amount+' {{ $basic->currency }}</td><td><a href="{{ route('staff-instalment-repayment-list') }}?custom='+ subcatObj.custom+'" class="btn btn-primary btn-sm btn-detail font-weight-bold text-uppercase"><i class="fa fa-list"></i> View List</a></td></tr>');
                    })
                }
            })
        });
