                                ...
                                <table class="table table-striped table-bordered table-hover zero-configuration">
                                    ...
                                    @foreach($history as $k => $p)
                                        <tr class="{{ $p->deleted_at != null ? 'bg-warning white' : '' }}">
                                            ...
                                            <td>
                                                @if($p->payment_type == 0)
                                                    <div class="badge badge-primary font-weight-bold text-uppercase">Deposit Amount</div>
                                                @elseif($p->payment_type == 1)
                                                    <div class="badge badge-warning font-weight-bold text-uppercase">Withdraw Amount</div>
                                                @else
                                                    <div class="badge badge-warning font-weight-bold text-uppercase">Expense Amount</div>
                                                @endif
                                            </td>
