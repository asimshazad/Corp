...
<head>
    ...
    <title>{{ $site_title }} | {{ __('common.invoice') }} </title>
...
<div class="wrapper">
    ...
    <section class="invoice">
        ...
        <div class="row invoice-logo">
            ...
            <div class="col-xs-6">
                <p class="text-center"> {{ __('common.invoice') }} : {{ $sell->custom }} <br>
                    <span class="invoice-extra-p">{{ \Carbon\Carbon::parse($sell->created_at)->format('dS M, Y - h:i A ') }}</span>
                </p>
                <div class="text-center">
                    <h5>
                        @if($sell->payment_type == 0)
                            <span class="label label-default"> {{ __('common.on_paid') }} </span>
                        @elseif($sell->payment_type == 1)
                            <span class="paymentStatus"> {{ __('common.due_payment') }} </span>
                        @else
                            <span class="label label-default"> {{ __('common.instalment') }}  </span>
                        @endif
                    </h5>
        ...
        <div class="row invoice-info">
            <div class="col-sm-6 col-xs-6">
                ...
                <h4 class="extra-h" style="margin-bottom: 3px;font-size: 14px;">{{ __('common.customer_detail') }} </h4>
            ...
            <div class="col-sm-6 col-xs-6">
                <div class="well extra-well extra-table2" style="padding: 10px;">
                    <h4 class="extra-h" style="margin-bottom: 3px;font-size: 14px;">{{ __('common.invoice_detail') }}</h4>
                    <ul class="list-unstyled" style="margin-bottom: 0;">
                        <li class="font-size-14"> <b>{{ __('common.issue_by') }} : </b>
                        ...
                        <li class="font-size-14"> <b>{{ __('common.total_amount') }}: </b>{{ $sell->total_amount }} {{ $basic->currency }}</li>
                        <li class="font-size-14"> <b>{{ __('common.pay_amount') }} : </b>{{ $sell->pay_amount }} {{ $basic->currency }}</li>
                    </ul>
                </div>
            </div>
            <!-- /.col -->
        ...
        <div class="row">
            <div class="col-xs-12 table-responsive">
                <table class="table table-striped extra-table2" style="margin-bottom: 0;">
                    <thead>
                    <tr>
                        <th>{{ __('sell.category') }}</th>
                        <th>{{ __('sell.serial_no') }}</th>
                        <th>{{ __('sell.p_desc') }}</th>
                        <th>{{ __('sell.sub') }}</th>
                    </tr>
                    </thead>
                    <tbody>
                ...
                <div class="table-responsive">
                    <table class="table extra-table" style="margin-bottom: 0px">
                        ...
                        @elseif($sell->payment_type == 1)
                            ...
                            <tr class="bg-grey bg-lighten-4">
                                <td class="text-bold-600">{{ __('sell.pay_amount') }}</td>
                                <td class="text-bold-600 text-right">{{ $sell->pay_amount }} - {{ $basic->currency }}</td>
                            </tr>
                            <tr class="bg-grey bg-lighten-4">
                                <td class="red text-bold-600">{{ __('sell.due_amount') }}Due Amount</td>
                        ...
                        @else
                            ...
                            <tr class="bg-grey bg-lighten-4">
                                <td class="red text-bold-600">{{ __('sell.due_amount') }}</td>
                                <td class="red text-bold-600 text-right">{{ $sell->due_amount }} - {{ $basic->currency }}</td>
                            </tr>
                        @endif
                        <tr>
                            <td colspan="2" class="text-capitalize text-center"><b></b>{{ __('sell.in_word') }} : {{ \App\TraitsFolder\CommonTrait::wordAmount($sell->total_amount) }} {{ $basic->currency }} Only.</td>
                        </tr>
                    </table>
                </div>
            </div>
